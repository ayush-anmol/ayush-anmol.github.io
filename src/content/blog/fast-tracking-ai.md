...
Most explanations of AI start in the middle — neural networks, parameters, GPUs — and hope you'll accept the beginning on faith. This post starts at the actual beginning: what it even means to learn. Everything else, up to and including ChatGPT, follows from that one idea.

## 1. Learning, knowledge, intelligence

Strip away the jargon and **learning is adjustment**: changing yourself based on experience. Try something, see what happens, adjust, repeat. That's it. Every learner — a toddler, a chess player, a neural network — runs the same loop; they differ only in what gets adjusted and how.

![The learning loop](/blog/fast-tracking-ai/01-learning-loop.svg)

Knowledge is what the loop leaves behind, and it comes in two flavors. **Explicit knowledge** you can write down: "Paris is the capital of France." **Implicit knowledge** you can't: how you recognize your friend's face, how you balance a bicycle, how you know "oh, great" is sarcastic. You know these things, but you couldn't hand someone the rules.
...
"Artificial intelligence" names an ambition: *making machines do things that would require intelligence if a human did them* — recognizing faces, understanding language, playing chess, driving a car. It says nothing about how. The last seventy years of AI history are essentially a sequence of attempted *hows*.
...
So flip the approach. Instead of writing rules, **show the machine thousands of examples and let it find the patterns itself**:
...
That's **machine learning** — the learning loop from section 1, mechanized. Note the relationship: AI is the goal, ML is one method for reaching it, and deep learning (coming shortly) is one family of ML that ended up eating everything.

Early ML had real wins — spam filters, Netflix and Amazon recommendations, basic image recognition. But it plateaued hard. It couldn't hold a conversation, couldn't understand a paragraph, couldn't recognize objects as reliably as a toddler.

## 5. Two bottlenecks and two winters

Why the plateau? Not bad ideas — missing ingredients.

**Not enough data.** Learning from examples needs *millions* of examples. Where do you get millions of labeled photos in 1990? The internet was in its infancy.

**Not enough compute.** Learning is arithmetic, staggering amounts of it. On the hardware of the day, serious training would have taken years.

Meanwhile the field kept promising human-level AI "in twenty years," and when it failed to deliver, funding and interest collapsed — twice. The 1970s brought the first **AI winter**; the expert-system bust of the late 80s and 90s brought the second. For a while, saying you worked on "AI" was a career liability.

![Booms and winters](/blog/fast-tracking-ai/03-booms-and-winters.svg)

Then, around 2012, all three missing ingredients arrived at once: the internet had produced **massive datasets**; **GPUs**, built for gaming, turned out to be perfect for ML's math; and researchers cracked how to train much deeper networks — **deep learning**. The field didn't just thaw; it detonated.
...
Who has the telescope — me or the man? Both readings are valid. Or compare *"fast-tracking the AI course"* with *"fast-tracking the course of AI"* — nearly identical strings, completely different meanings. Computers need precision; human language is ambiguous, contextual, and messy on purpose.
...
Attention killed three problems at once: **no forgetting** (every word sees every word, at any distance), **speed** (all positions process in parallel — perfect for GPUs), and **context** (each word's representation is built from the words that actually matter around it). The Transformer is what you get when you commit fully: no recurrence, no convolutions, attention all the way down. It's the foundation of every modern large language model.

*If you want the full story of attention — multi-head, KV caches, FlashAttention, and everything models ship in 2026 — I wrote a [dedicated deep dive](/blog/attention-and-its-variants).*
...
The natural objection: how can *autocomplete* understand anything? The answer is that predicting the next word well is a brutally demanding task. To predict the word after "the capital of France is," you need **facts**. To continue grammatical sentences, you need **syntax** — internalized, not memorized. To continue "so the answer must be…", you need to have followed the **reasoning** that came before. Prediction doesn't sidestep understanding; done well enough, it *requires* it.
